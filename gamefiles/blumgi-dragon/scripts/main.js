...
window.DOMElementHandler = class extends self.DOMHandler {
    ...
    _OnUpdatePosition(a) {
        if (this._autoAttach) {
            ...
            b.HadFirstUpdate() || (b.SetHadFirstUpdate(),
            b.GetVisibleFlag() && (c.style.display = ""))
        }
    }
    _OnUpdateState(a) {
...
RuntimeInterface$jscomp$1.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    Attach() {
        ...
        this._iRuntime.UsesWorker() && "undefined" !== typeof window.onpointerrawupdate && self === self.top ? window.addEventListener("pointerrawupdate", b=>this._OnPointerRawUpdate(b)) : window.addEventListener("pointermove", b=>this._OnPointerEvent("pointermove", b)),
    ...
    _OnScreenReaderTextEvent(a) {
        var b = a.type;
        "create" === b ? (b = document.createElement("p"),
        b.id = "c3-sr-" + a.id,
    ...
    async _GetSafeAreaInset() {
        ...
        if (a)
            try {
                ...
                return console.error("[Construct] Failed to get Android safe area inset: ", c),
                [0, 0, 0, 0]
            }
        else {
            var b = document.body;
...
self.AudioDOMHandler = class extends self.DOMHandler {
    ...
    _ReconnectEffects(a) {
        ...
        for (const d of this.audioInstancesByEffectTag(a))
            d.Reconnect(b);
    ...
    *audioInstancesByEffectTag(a) {
    ...
    _Stop(a) {
        a = a.tags;
        for (const b of this.audioInstancesMatchingTags(a))
            b.Stop()
    }
    _StopAll() {
        for (const a of this._audioInstances)
    ...
    async _FadeVolume(a) {
        ...
        for (const e of this.audioInstancesMatchingTags(b))
            e.FadeVolume(c, d, a);
        this._MaybeStartTicking()
    }
    _SetMasterVolume(a) {
    ...
    _OnTick(a) {
        ...
        if ((b = a.listenerOrientation) && (this._lastListenerOrientation[0] !== b[0] || this._lastListenerOrientation[1] !== b[1] || this._lastListenerOrientation[2] !== b[2] || this._lastListenerOrientation[3] !== b[3] || this._lastListenerOrientation[4] !== b[4] || this._lastListenerOrientation[5] !== b[5])) {
            for (let c = 0; 6 > c; ++c)
                this._lastListenerOrientation[c] = b[c];
            this._audioContext.listener.setOrientation(...this._lastListenerOrientation)
        }
    ...
    async _AddEffect(a) {
        ...
        for (const f of c) {
            if ("filter" === b)
                c = new self.C3AudioFilterFX(this,...d);
            else if ("delay" === b)
                c = new self.C3AudioDelayFX(this,...d);
                ...
                c = new self.C3AudioFlangerFX(this,...d);
            else if ("phaser" === b)
                c = new self.C3AudioPhaserFX(this,...d);
            else if ("gain" === b)
                c = new self.C3AudioGainFX(this,...d);
            else if ("stereopan" === b)
                c = new self.C3AudioStereoPanFX(this,...d);
            else if ("tremolo" === b)
                c = new self.C3AudioTremoloFX(this,...d);
            else if ("ringmod" === b)
                c = new self.C3AudioRingModFX(this,...d);
            else if ("distortion" === b)
                c = new self.C3AudioDistortionFX(this,...d);
            else if ("compressor" === b)
                c = new self.C3AudioCompressorFX(this,...d);
            else if ("analyser" === b)
                c = new self.C3AudioAnalyserFX(this,...d);
            else
    ...
    async _OnLoadState(a) {
        ...
        if (Array.isArray(c)) {
            for (d = 0; 6 > d; ++d)
                this._lastListenerOrientation[d] = c[d];
            this._audioContext.listener.setOrientation(...this._lastListenerOrientation)
        }
        this._isSilent = a.isSilent;
        this._iRuntime.SetSilent(this._isSilent);
...
self.C3AudioInstance = class {
    ...
    SetPan(a, b, c, d, e, f, g) {
        ...
        this._pannerConeParams[0] !== a(e) && (this._pannerConeParams[0] = a(e),
        this._pannerNode.coneInnerAngle = a(e)),
        this._pannerConeParams[1] !== a(f) && (this._pannerConeParams[1] = a(f),
        this._pannerNode.coneOuterAngle = a(f)),
        this._pannerConeParams[2] !== g && (this._pannerConeParams[2] = g,
